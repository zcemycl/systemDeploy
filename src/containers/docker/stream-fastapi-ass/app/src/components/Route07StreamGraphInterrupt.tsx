...
export default function Route07StreamGraphInterrupt({ apiBaseUrl }: Props) {
  ...
  async function loadDefinition() {
    setError('')
    try {
      const response = await fetch(`${apiBaseUrl}/stream-graph-07/definition`)
  ...
  async function runGraph() {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const eventSource = new EventSource(`${apiBaseUrl}/stream-graph-07`)
            ...
            if (payload.type === 'review_submitted') {
              ...
              setNodeDetails((prev) => ({
                ...
                [payload.node.id]: {
                  ...
                  stateSnapshot: prev[payload.node.id]?.stateSnapshot,
                },
              }))
  ...
  async function submitDecision(action: 'approve' | 'reject' | 'edit') {
    ...
    setError('')
    try {
      const response = await fetch(`${apiBaseUrl}/stream-graph-07/review-decision`, {
        method: 'POST',
