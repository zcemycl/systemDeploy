...
resource "null_resource" "api_ecr_push" {
  triggers = {
    file1 = filemd5("../naive-3tier/src/backend/Dockerfile")
    file2 = filemd5("../naive-3tier/src/backend/main.py")
    file3 = filemd5("../naive-3tier/src/backend/requirements.txt")
  }

  provisioner "local-exec" {
    command = "cd ../naive-3tier/src/backend/ && docker build --platform linux/amd64 -t ${module.ecr.api_image}:latest -f Dockerfile ."
  }
  provisioner "local-exec" {
