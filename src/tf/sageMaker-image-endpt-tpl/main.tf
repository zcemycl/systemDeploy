...
resource "null_resource" "push_docker_image" {
  ...
  provisioner "local-exec" {
    command = "docker tag test-dummy-sagemaker-image:latest ${aws_ecr_repository.this.repository_url}:latest >> info.txt"
  }
  provisioner "local-exec" {
...
resource "aws_sagemaker_endpoint_configuration" "this" {
  name = "${local.prefix}-sagemaker-conf"
...
resource "aws_sagemaker_endpoint" "this" {
  name                 = "${local.prefix}-sagemaker-endpt"
